...
//
// {  body: '{    \"name\" : \"billy\", \"password\" : \"pw123\"}'
//
// }
//
   ...
   // ready to go for CORS. To make this a completed HTTP response, you only need to add a statusCode and a body.
    ...
    let CheckNameUnique = (value) => { //make a function like this every time you want to check the database for a key and return a specific value
        return new Promise((resolve, reject) => { //promise that you will get info whe nthe fuction returns 
            pool.query("SELECT * FROM designer WHERE name=?", [value], (error, rows) => { //query the database for a specific name, the ncall back function for the error or the row 
    ...
    let InsertNewDesigner = (name, password) => {
        return new Promise((resolve, reject) => {
            pool.query("INSERT INTO designer (name, password) VALUES(?,?)", 
               [name, password], (error, rows) => {
    ...
    try {
        ...
        if (name_value != true){ //|| info.name == null || info.password == null) { 
            ...
            response.error = "Designer already exists";
        } else {
            // otherwise SUCCESS!
            console.log("made it to adding a new designer"); 
            response.statusCode = 200;
            let fullresult = await InsertNewDesigner(info.name, info.password); //add designer to the table and return the designerID
            let result = await GetID(info.name); 
            console.log("result: " + result); 
            response.result = JSON.stringify(result); 
            console.log("desinger supposed to be added and result set"); 
        }
        //***check fro errors
    } catch (error) {
